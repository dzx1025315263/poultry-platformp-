...
import { Download, FileSpreadsheet, Heart, Filter, CheckCircle } from "lucide-react";
...
export default function ExportPage() {
  ...
  const [lastExport, setLastExport] = useState<{ count: number; time: string } | null>(null);
  ...
  // Export queries - only enabled when manually triggered via refetch
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (exportType === "companies") {
        ...
        const filename = "企业数据" + filterStr + "_" + dateStr + ".csv";

        downloadCsv(rows, filename);
        setLastExport({ count: data.length, time: new Date().toLocaleTimeString("zh-CN") });
        toast.success("成功导出 " + data.length + " 家企业数据");
      } else {
        const result = await exportFavoritesQuery.refetch();
        const data = result.data as any[];
        if (!data?.length) {
          toast.error("收藏夹中没有符合条件的数据");
          setIsExporting(false);
          return;
        }
        ...
        const filename = "收藏夹导出" + statusStr + "_" + dateStr + ".csv";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">按条件筛选并导出企业数据或收藏夹为CSV格式，可直接用Excel打开</p>
      </div>
          ...
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-green-800">
                上次导出：<span className="font-medium">{lastExport.count} 条数据</span>，时间 {lastExport.time}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card
          className={"cursor-pointer transition-all hover:shadow-md " + (exportType === "companies" ? "ring-2 ring-primary shadow-md" : "")}
        ...
        <Card
          className={"cursor-pointer transition-all hover:shadow-md " + (exportType === "favorites" ? "ring-2 ring-primary shadow-md" : "")}
      ...
      <Card>
        <CardContent className="pt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">导出格式：CSV（UTF-8 BOM编码，可直接用Excel打开）</p>
          </div>
          <Button onClick={handleExport} disabled={isExporting} size="lg" className="gap-2 shrink-0">
            <Download className="h-4 w-4" />
            {isExporting ? "导出中..." : "导出CSV文件"}
          </Button>
        </CardContent>
      </Card>
    </div>
